...
```{r vcf}
vcf_file <- file.path(
  "/hpcfs/users/a1647910/211001_APOE_Mm/analysis-variants_new/results/07_variants/6_select/all_samples.vcf.gz"
)
...
```{r}
snvs <- rowRanges(vcf)
mcols(snvs) <- mcols(snvs) %>%
  as_tibble() %>%
  dplyr::select(-paramRangeID)
# genome(snvs) <- genome(genes)
snvs <- snvs %>%
  as.data.frame() %>%
  droplevels() %>%
  GRanges()
...
clust_filt <- snvs %>%
  as_tibble() %>%
  dplyr::filter(FILTER == "SnpCluster") %>%
  mutate(clust = ifelse(FILTER == "SnpCluster", 1, 0)) %>%
  GRanges()
```
...
```{r allele_file}
allele_file <- file.path(here(), "files", "allele_stats_test.Rds")
```
...
```{r allele_dist}
dist_winSize <- 11
allele_dist_path <- here("files", "allele_dist_test.Rds")
...
dist_winRanges_path <- file.path(here(), "files", "dist_winRanges_test.Rds")
...
masterGR_path <- here("files/masterGR_test.Rds")
...
plot_allele_dist(
  ...
  dist = allele_dist[[ind]], dist_type = c("b"), dist_trans = 1,
