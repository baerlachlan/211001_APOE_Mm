...
```{r}
genes_thresh <- lapply(genes_diversity, function(genes){
  lapply(thresholds, function(threshVal){
    genes %>%
      as_tibble() %>%
      dplyr::filter(diversity <= threshVal) %>%
      dplyr::mutate(threshold = threshVal)
  })
})
```

```{r}
lapply(seq_along(genes_thresh), function(ind){
  genes_thresh[ind] %>%
    bind_rows() %>%
    group_by(threshold) %>%
    summarise(nGenes = n()) %>%
    mutate(group = names(genes_thresh[ind]))
}) %>%
  bind_rows() %>%
  ggplot(aes(threshold, nGenes)) +
  geom_bar(stat = "identity", colour = "black", fill = "grey50") +
  geom_text(aes(threshold, nGenes, label = nGenes), nudge_y = 500) +
  scale_x_continuous(breaks = seq(0, 1, 0.1)) +
  labs(x = "Diversity threshold", y = "Genes") +
  facet_wrap(~group)
```

```{r}
topTables_thresh <- lapply(names(genes_diversity), function(contrast){
  lapply(thresholds, function(threshVal){
    genes_passed <- genes_diversity[[contrast]]$gene_id[
      genes_diversity[[contrast]]$diversity <= threshVal
    ]
    fit <- dgeList[genes_passed,] %>%
      calcNormFactors() %>%
      estimateDisp(design) %>%
      glmFit()
    glmLRT(fit, contrast = contrasts[,contrast]) %>%
      topTags(n = Inf) %>%
      .[["table"]] %>%
      as_tibble() %>%
      arrange(PValue) %>%
...
```{r}
tt <- topTables_chosen$APOE4v3_male
man <- tt %>%
  mutate(mid = (start + end) / 2) %>%
  dplyr::select(gene_id, gene_name, chromosome, mid, FDR) %>%
  dplyr::filter(!(chromosome %in% c("X", "Y", "MT"))) %>%
...
axis <- man %>%
  group_by(chromosome) %>%
  summarise(center = (max(midCum) + min(midCum)) / 2)
b <- ggplot(man, aes(x = midCum, y = -log10(PValue))) +
  geom_point(
    data = man %>%
      dplyr::filter(!DE),
    aes(colour = colour),
...
```{r}
ggsave(
  "~/phd/publications/fisher/figures/figure_3.png",
  device = "png",
  width = 18,
  height = 12,
  units = "cm"
