...
```{r}
corr <- interGeneCorrelation(dgeList$counts, design)
```
  ...
  apply(MARGIN = 2, function(contrast){
    cpm(dgeList$counts, log = TRUE) %>%
      camera(
        ...
        design = design,
        contrast = contrast,
        inter.gene.cor = corr$correlation
      ) %>%
      rownames_to_column("Pathway") %>%
      as_tibble() %>%
      mutate(
...
```{r goseq_pwf}
# pwf <- map2(topDE, genes_diss[1:4], function(topTable, geneDiss){
#   topTable %>%
#     left_join(as.data.frame(geneDiss)[,c("gene_id", "diss")]) %>%
#     dplyr::filter(!is.na(diss)) %>%
#     with(
#       nullp(
#         DEgenes = structure(as.integer(DE), names = gene_id),
#         genome = "danRer11",
#         id = "ensGene",
#         bias.data = diss,
#         plot.fit = TRUE
...
### APOE 2v3 females {.tabset .tabset-pills}

#### `camera`

```{r}
datatable(hm_camera[[1]])
```
...
### APOE 2v3 males {.tabset .tabset-pills}

#### `camera`

```{r}
datatable(hm_camera[[2]])
```
...
### APOE 4v3 females {.tabset .tabset-pills}

#### `camera`

```{r}
datatable(hm_camera[[3]])
```
...
### APOE 4v3 males {.tabset .tabset-pills}

#### `camera`

```{r}
datatable(hm_camera[[4]])
```
...
```{r hm_camera_eqtl}
hm_camera_eqtl <- colnames(contrasts) %>%
  sapply(function(cont){
    biased_genes <- genes_diss[[cont]] %>%
      as_tibble() %>%
      dplyr::filter(diss > 0.5) %>%
...
```{r kg_camera_eqtl}
kg_camera_eqtl <- colnames(contrasts) %>%
  sapply(function(cont){
    biased_genes <- genes_diss[[cont]] %>%
      as_tibble() %>%
      dplyr::filter(diss > 0.5) %>%
...
```{r wk_camera_eqtl}
wk_camera_eqtl <- colnames(contrasts) %>%
  sapply(function(cont){
    biased_genes <- genes_diss[[cont]] %>%
      as_tibble() %>%
      dplyr::filter(diss > 0.5) %>%
