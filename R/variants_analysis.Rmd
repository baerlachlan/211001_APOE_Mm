...
```{r chrInfo}
chrInfo <- getChromInfoFromEnsembl("GRCm39", release = "104") %>%
  dplyr::filter(name %in% primary_chrs)
```
...
```{r}
chrGR <- chrInfo[,c("name", "length")] %>%
  dplyr::rename(seqnames = name, end = length) %>%
  mutate(start = 1) %>%
  GRanges()
```
...
apoeGR <- genes %>%
  as_tibble() %>%
  dplyr::filter(str_detect(gene_name, "Apoe")) %>%
  GRanges()
apoe_mid <- ranges(apoeGR) %>%
  as.data.frame() %>%
  with(start + end / 2)
```

```{r}
kp <- plotKaryotype(chrGR, chromosomes = primary_chrs)
kpPlotDensity(kp, snvGR, col = "black", window.size = 1e3)
kpPlotRegions(kp, apoeGR, col = "red")
```

## Similarity

```{r}
vcf_files <- list.files("/hpcfs/users/a1647910/211001_APOE_Mm_ASE/07_variants/4_selected", pattern = ".vcf.gz$", full.names = TRUE)
profiles <- lapply(vcf_files, function(x){
  sample <- basename(x) %>%
    str_remove(".vcf.gz$")
  create_profile(x, sample = sample,  filter_gd = FALSE, filter_vc = FALSE, filter_mt = FALSE, filter_ns = FALSE, filter_pd = FALSE)
})
```

```{r}
compare_profiles(profiles[[1]], profiles[[2]])
```
